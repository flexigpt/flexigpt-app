...
func NewInstaller(
	...
	if dependencies.Agents == nil || dependencies.Packages == nil {
		return nil, fmt.Errorf(
			"%w: built-in Agent installer dependencies are incomplete",
			basespec.ErrInvalid,
		)
	}
...
func (*Installer) BuiltInName() string {
	return agentDomain.BuiltInInstallerName
}
...
func (i *Installer) installPreparedPackage(
	...
	if _, err := i.agents.InstallBuiltInAgentPackage(
		ctx,
		...
		return fmt.Errorf(
			"install built-in Agent package %q: %w",

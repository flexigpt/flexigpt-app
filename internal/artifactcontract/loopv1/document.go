...
const (
	LoopType          = artifactcontract.TypeLoop
	LoopSchemaID      = "artifact.loop.v1"
	LoopSchemaVersion = artifactcontract.APIVersionV1
)
...
type LoopDocument struct {
	...
	Body          *artifactcontract.Entry       `json:"body,omitempty"`
	MaxIterations int                           `json:"maxIterations,omitempty"`
	Until         *artifactcontract.OutputMatch `json:"until,omitempty"`
}
...
func (v LoopDocument) validate(
	...
	if v.MaxIterations == 0 {
		...
		raw, err := artifactcontract.CanonicalDocumentJSON(v)
		if err != nil {
			return err
		}

...
const (
	MCPType          = artifactcontract.TypeMCP
	MCPSchemaID      = "artifact.mcp.v1"
	MCPSchemaVersion = artifactcontract.APIVersionV1
)
...
func (v MCPDocument) validate(requireName bool) error {
	...
	if v.URL != "" {
		if err := artifactcontract.ValidateAbsoluteURL(
			"MCP URL",
			v.URL,
		); err != nil {
			return err
	...
	if err := artifactcontract.ValidateTextSlice(
		"MCP args",
		v.Args,
		basespec.MaxURIBytes,
	); err != nil {
	...
	if err := artifactcontract.ValidateStringMap(
		"MCP env",
		v.Env,
	); err != nil {
		return err
	...
	return artifactcontract.ValidateTextSlice(
		"MCP include prompts",
		v.Include.Prompts,
		basespec.MaxLogicalNameBytes,
	)

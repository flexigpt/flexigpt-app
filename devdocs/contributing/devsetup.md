...
- Install base prerequisites `node`, `go`, `task (taskfile.dev)`. Pinned versions of these can be found in file at repo root named: `.tool-versions`. Recommended: use `asdf` to manage the base prerequisites. If you have asdf installed you can do `asdf install` to install all required versions of this project.
...
- Install go tools prerequisites:
  - Run the script as `task install-go-tools`
  - Wails issues:

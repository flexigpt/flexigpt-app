...
- If Microsoft Defender SmartScreen shows a **Windows protected your PC** warning:
  ...
  - Select **More info**.
  - Select **Run anyway**.
  - If **Run anyway** is not available, your device or organization policy may block unsigned apps. Use a device where you are allowed to install unsigned applications or contact your administrator.
...
Run the app with: `flatpak run io.github.flexigpt.client`

Known first-launch issue on Linux with Nvidia drivers:

- If you use Nvidia proprietary drivers, the app may open a blank window and close.
- Try launching it with: `flatpak run --env=WEBKIT_DISABLE_COMPOSITING_MODE=1 io.github.flexigpt.client`

If that works, the issue is likely the known WebKit rendering problem on some Linux setups.

For platform-specific storage locations, what is stored locally, and where app data is kept, see [Privacy, Data, and Troubleshooting](./07-privacy-data-and-troubleshooting.md).

...
- [ ] google genai sdk integration from inference.
  - [ ] role model mapping verify. thoughts are not visible in ui as of now.
  - [ ] may be if there is jsut thought response but no tool calls, we may want to show thoughts or may be even if there are tool calls showing thoughts is not a bad idea ?? need to analyze carefully. may be thought with calls can have a thoughts button in single row to show thoughts separatly rather than detaisl or detaisl can extract thoughts and show even today.
  - [ ] some issue with tool calls and parts streaming i think i am seeing some streams etc but no tool calls in ui neither response.
  - [ ] tool call was visible in non streaming mode but again thought signature seems missing in submit.
  - [ ] signature seems to be not wired correctly.
